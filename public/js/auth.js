/* ============================================================
   VELVET BEAUTY — auth.js (MongoDB API version)
...
function switchRole(role, btn) {
  ...
  if (role === 'employee') {
    const h1 = document.getElementById('login-code-hint'), h2 = document.getElementById('reg-code-hint');
...
async function handleLogin(e) {
  ...
  const result = await VBAuth.login(email, password, authRole, code);
...
async function handleRegister(e) {
  ...
  const result = await VBAuth.register({ firstName, lastName, email, phone, password, role: authRole, accessCode: code });
